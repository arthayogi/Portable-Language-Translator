...
import talkifyServiceKR from "../components/talkifyService_KR";

export default function TLKR_Page() {
  ...
  const TextToSpeech = () => {
    ...
    const handleSpeak = async () => {
      const url = await talkifyServiceKR.getSpeechUrl(text);
      setAudioUrl(url);
    };
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const { data } = await axios({
        method: "POST",
        url: "http://18.141.138.126/translate-kr",
        headers: {
