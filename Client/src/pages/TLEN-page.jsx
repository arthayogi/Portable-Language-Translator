...
import talkifyServiceEN from "../components/talkifyService_EN";
import { useDispatch, useSelector } from "react-redux";
// import TextToSpeech from "../components/text-to-speech";

export default function TLEN_Page() {
  ...
  const TextToSpeech = () => {
    ...
    const handleSpeak = async () => {
      const url = await talkifyServiceEN.getSpeechUrl(text);
      setAudioUrl(url);
    };
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const { data } = await axios({
        method: "POST",
        url: "http://18.141.138.126/translate-en",
        headers: {
